...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        currentScore = dice()
        existsOne = currentScore == 1 or existsOne
        s += currentScore
        pass
    if existsOne:
        return 1
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if not num_factors(score) in (4, 3):
        return score
    score += 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    default_num = strategy(0, goal-1)
    for i in range(goal):
        for j in range(goal):
            if not default_num == strategy(i, j):
                return False
            pass
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, samples_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    def _func(*args):
        nonlocal original_function, samples_count
        s = 0
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for i in range(1, 11):
        avg_data[i] = func(i, dice)
        pass
    max_avg = max(avg_data.values())
    for i in range(1, 11):
        if avg_data[i] == max_avg:
            return i
        pass
